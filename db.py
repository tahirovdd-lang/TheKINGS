    ...
    with get_conn() as con:
        cur = con.execute("""
            INSERT INTO appointments(
                ...
                master_id, master_name, date, time,
                duration_min, total_price, services_json,
                comment, status
            )
            VALUES(?,?,?,?,?,?,?,?,?,?,?,?)
        """, (
            data["user_id"], data.get("user_name",""), data.get("user_phone",""),
            data["master_id"], data.get("master_name",""),
            data["date"], data["time"],
            data.get("duration_min",0), data.get("total_price",0),
